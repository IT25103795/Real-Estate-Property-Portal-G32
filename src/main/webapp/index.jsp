...
button:active { transform: scale(0.96); transition: transform .1s ease; }
...
.hero-left {
  ...
  animation: heroFadeIn .9s cubic-bezier(.16,1,.3,1) both;
}
@keyframes heroFadeIn { from { opacity:0; transform: translateX(-40px) scale(.97); filter: blur(4px); } to { opacity:1; transform: none; filter: blur(0); } }
.hero-eyebrow {
...
.hero-right {
  ...
  animation: heroFadeInRight .9s .12s cubic-bezier(.16,1,.3,1) both;
}
@keyframes heroFadeInRight { from { opacity:0; transform: translateX(40px) scale(.97); filter: blur(4px); } to { opacity:1; transform: none; filter: blur(0); } }
.hero-main-img {
...
@keyframes floatCard { 0%,100% { transform: translateY(0) rotate(0deg); } 30% { transform: translateY(-10px) rotate(.5deg); } 70% { transform: translateY(-5px) rotate(-.4deg); } }
...
.prop-card:hover {
  ...
  box-shadow: 0 20px 60px rgba(0,0,0,.13), 0 4px 16px rgba(26,86,219,.08);
  transform: translateY(-6px) scale(1.01);
}
/* Stagger card animations */
.prop-card:nth-child(1) { animation-delay: .05s; }
.prop-card:nth-child(2) { animation-delay: .12s; }
.prop-card:nth-child(3) { animation-delay: .19s; }
.prop-card:nth-child(4) { animation-delay: .26s; }
.prop-card:nth-child(5) { animation-delay: .33s; }
...
.prop-card--list {
  ...
  background: var(--bg); border: 1.5px solid var(--line);
  transition: all var(--t); animation: cardIn .65s cubic-bezier(.16,1,.3,1) both;
...
.prop-card--spotlight {
  ...
  justify-content: flex-end; animation: cardIn .65s cubic-bezier(.16,1,.3,1) both;
...
.prop-card--magazine {
  ...
  border: 1.5px solid var(--line); animation: cardIn .65s cubic-bezier(.16,1,.3,1) both;
...
.prop-card--minimal {
  ...
  animation: cardIn .65s cubic-bezier(.16,1,.3,1) both; transition: all var(--t);
}
...
.skeleton { background: linear-gradient(90deg, var(--bg2) 25%, var(--bg3) 50%, var(--bg2) 75%); background-size: 200% 100%; animation: shimmer 1.2s ease-in-out infinite; border-radius: var(--r); }
...
.skeleton-img { height: 230px; width: 100%; }
...
.skeleton-line.price { height: 22px; width: 55%; }
...
.rev-dropdown.show { display: block; animation: cardIn .65s cubic-bezier(.16,1,.3,1); }
.rev-drop-item {
...
.nav-cart-btn:hover { border-color: var(--red); color: var(--red); }
.nav-cart-badge {
    background: var(--red);
    color: #fff;
    font-size: 0.6rem;
...
.cart-panel-close:hover { background: var(--bg3); color: var(--ink); }
...
.cart-panel-list::-webkit-scrollbar { width: 4px; }
.cart-panel-list::-webkit-scrollbar-thumb { background: var(--line); border-radius: 10px; }
...
.cart-item-loc { font-size: 0.75rem; color: var(--ink3); margin-top: 2px; }
.cart-item-price { font-size: 0.8rem; font-weight: 700; color: var(--accent); margin-top: 3px; }
.cart-item-del {
    background: none;
    border: none;
    cursor: pointer;
...
.cart-item-del:hover { color: var(--red); background: var(--bg3); }
...
.cart-panel-empty .empty-icon { font-size: 2.5rem; }
...
.cart-footer-stat strong { color: var(--ink); }
.cart-btn-primary {
    width: 100%;
...
.cart-btn-secondary:hover { border-color: var(--red); color: var(--red); }
</style>
</head>
<body>
  ...
  <ul class="nav-links">
    ...
    <li><a href="#" onclick="showPage('about')" id="nav-about">About</a></li>
      ...
      <div class="nav-actions">
            ...
            <%-- ── SAVED CART BUTTON (BUYERS ONLY) ── --%>
            <c:if test="${sessionScope.loggedRole == 'BUYER'}">
                <button class="nav-cart-btn" onclick="toggleCart(event)" title="My Saved Properties">
                    ❤️ Saved <span class="nav-cart-badge" id="cart-badge">0</span>
                </button>
            </c:if>
...
<div class="cart-panel" id="cart-panel">
    <div class="cart-panel-header">
        <div class="cart-panel-title">❤️ Saved Properties <span class="nav-cart-badge" id="cart-badge-panel" style="display:inline;background:var(--red);color:#fff;font-size:0.65rem;padding:2px 7px;border-radius:10px;margin-left:4px;">0</span></div>
        <button class="cart-panel-close" onclick="closeCart()">&#x2715;</button>
    </div>

    <div class="cart-panel-list" id="cart-panel-list">
        <div class="cart-panel-empty" id="cart-empty">
            <div class="empty-icon">🏠</div>
            <div>No saved properties yet.</div>
            <div style="font-size:0.8rem;">Browse listings and tap ❤️ to save!</div>
        </div>
    </div>
    ...
    <div class="cart-panel-footer" id="cart-footer" style="display:none;">
        ...
        <button class="cart-btn-primary" onclick="closeCart(); window.location.href='buyerDashboard'">View all in Dashboard</button>
        <button class="cart-btn-secondary" onclick="cartClearAll()">🗑️ Clear all</button>
    </div>
</div>
...
<script>
    ...
    // Build cart badge immediately after properties are ready
    ...
    function buildCartPanel() {
        ...
        const badgePanel = document.getElementById('cart-badge-panel');
        const countLabel = document.getElementById('cart-count-label');

        if (!list) return;

        // Get saved properties from allProperties that are in favPropertyIds
        const saved = (window.properties || []).filter(p => window.favPropertyIds && window.favPropertyIds.has(p.id));
        ...
        list.querySelectorAll('.cart-item-card').forEach(el => el.remove());
        ...
        if (count === 0) {
            ...
            saved.forEach(p => {
                const priceStr = '$' + Number(p.price).toLocaleString() + (p.status === 'For Rent' ? '/day' : '');
                const card = document.createElement('div');
                card.className = 'cart-item-card';
                card.id = 'cart-card-' + p.id;
                card.innerHTML =
                    '<img class="cart-item-thumb" src="' + p.image + '" alt="' + p.title + '" onerror="this.style.background=\'var(--bg3)\'">' +
                    '<div class="cart-item-info">' +
                        '<div class="cart-item-name">' + p.title + '</div>' +
                        '<div class="cart-item-loc">📍 ' + p.location + '</div>' +
                        '<div class="cart-item-price">' + priceStr + '</div>' +
                    '</div>' +
                    '<button class="cart-item-del" title="Remove" onclick="event.stopPropagation(); cartRemoveItem(\'' + p.id + '\')">✕</button>';
                card.addEventListener('click', function() { closeCart(); openDetail(p.id); });
    ...
    function cartRemoveItem(propertyId) {
        ...
        .then(data => {
            if (data.status === 'ok') {
                ...
                document.querySelectorAll('.heart-btn').forEach(btn => {
                    if (btn.closest('[onclick*="' + propertyId + '"]') || (btn.getAttribute('onclick') || '').includes(propertyId)) {
    ...
    function cartClearAll() {
        ...
        form.method = 'POST';
        form.action = 'bulkRemoveFavorite';
        const mode = document.createElement('input');
        mode.type = 'hidden'; mode.name = 'mode'; mode.value = 'all';
